 ...
 */
public class LRUCacheWithQueue {

    private Map<Integer, Integer> cacheMap;

    private int capacity;

    private Queue<Integer> ttlQueue;

    public LRUCacheWithQueue(int capacity){
        this.cacheMap = new HashMap<>(capacity);
        this.capacity = capacity;
        ttlQueue = new ArrayDeque<>(capacity);
    }

    public int get(int key){
        Integer value = cacheMap.get(key);
        if(value == null){
            //System.out.println("not get key:" + key + ", value:" + value);
            return -1;
        }else{
    ...
    private void ttl(Integer key) {
        Integer headerKey = ttlQueue.peek();
        //队列已满 队首满足
        if(ttlQueue.size() == capacity
                && Integer.compare(key, headerKey) == 0){
            //出队
            ttlQueue.poll();
        }else{
            //删除
    ...
    public void put(int key, int value){
        ...
        System.out.println("put key:" + key + ", value:" + value
                ...
                + ", ttlQueue:" + ttlQueue + "ttlQueue size:" + ttlQueue.size());
    }

    private void removeTTLHeader() {
        Integer header = ttlQueue.poll();
        if(header != null){
            //覆盖删除
    ...
    private static void testFromFile() throws IOException {
        ...
        try {
            ...
            LRUCacheWithQueue cache = new LRUCacheWithQueue(Integer.valueOf(valueArray[0].substring(1)));
    ...
    private static void test4() {
        LRUCacheWithQueue cache = new LRUCacheWithQueue( 10 );
    ...
    private static void test3() {
        LRUCacheWithQueue cache = new LRUCacheWithQueue( 2 );
